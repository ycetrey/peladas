...
const CLAUDE_MD_FALLBACKS = {
  ...
  skills: 'No project skills found. Add skills to any of: `.cursor/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, or `.codex/skills/` with a `SKILL.md` index file.',
};
...
// ─── .cursor/rules/ Section Generators ─────────────────────────────────────────────
 ...
 * (name + description) for each. Returns a table summary for .cursor/rules/ so
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  let targetPath;
  if (options.global) {
    targetPath = path.join(os.homedir(), '.claude', '.cursor/rules/');
  } else if (options.output) {
    targetPath = path.isAbsolute(options.output) ? options.output : path.join(cwd, options.output);
  } else {
    // Read claude_md_path from config, default to ./.cursor/rules/
    let configClaudeMdPath = './.cursor/rules/';
    try {
