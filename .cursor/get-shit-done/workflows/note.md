...
- **Global scope**: `D:/Fontes/peladas/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Fontes/peladas/.cursor/notes/`)
...
2. Glob `D:/Fontes/peladas/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (D:/Fontes/peladas/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Fontes/peladas/.cursor/notes/` — works in any directory
