...
4. **CORS / origem:** API Nest deve permitir `http://localhost:3002` (e variável de ambiente opcional) em **`enableCors`** para o browser falar com `http://localhost:3001`; alternativa rejeitada para v1: apenas proxy Next sem tocar na API (duplica config Docker).
5. **Erros de negócio:** Reutilizar contrato `{ error: { code, message } }`; mostrar `message` ao utilizador e `code` em texto secundário ou tooltip.
6. **Posição na inscrição:** Select ou botões com valores do enum `PlayerPosition` alinhados ao DTO da API.
...
- Testes E2E Playwright no CI (opcional como stretch em 05-03).
